...
const TRANSLATIONS = {
	...
	en: {
		...
		notFoundText: "The coffee bean you're looking for doesn't exist or has been removed.",
		originLabel: 'Origin',
		regionLabel: 'Region',
	...
	de: {
		...
		notFoundText: 'Die Kaffeebohne, die Sie suchen, existiert nicht oder wurde entfernt.',
	...
	fr: {
		...
		notFoundText: "Le grain de café que vous recherchez n'existe pas ou a été supprimé.",
		originLabel: 'Origine',
		regionLabel: 'Région',
...
export function meta({ data, params }: Route.MetaArgs) {
	...
	const imageUrl = bean.regionImage ? urlFor(bean.regionImage)?.width(800).url() : null
			...
			<div className={styles.container}>
				...
				<article className={styles.article}>
					<header className={styles.header}>
						...
						{(origin || region) && (
							<p className={styles.origin}>{[origin, region].filter(Boolean).join(', ')}</p>
						)}
