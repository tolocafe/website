...
const TRANSLATIONS = {
	...
	en: {
		...
		notFoundText: "The location you're looking for doesn't exist or has been removed.",
	...
	de: {
		...
		notFoundText: 'Der Standort, den Sie suchen, existiert nicht oder wurde entfernt.',
	...
	fr: {
		...
		notFoundText: "L'emplacement que vous recherchez n'existe pas ou a été supprimé.",
		addressLabel: 'Adresse',
		hoursLabel: 'Horaires',
...
export function meta({ data, params }: Route.MetaArgs) {
	...
	const imageUrl = location.image ? urlFor(location.image)?.width(1200).url() : null
...
export default function LocationDetail({ loaderData }: Route.ComponentProps) {
	...
	const imageUrl = location.image ? urlFor(location.image)?.width(1200).height(675).url() : null
				...
				<article className={styles.article}>
					...
					{imageUrl && (
						<div className={styles.imageWrapper}>
							<img src={imageUrl} alt={location.image?.alt || name} className={styles.image} />
						</div>
					)}
